...
function queueChars(room)
  for i, v in ipairs(currentLevel.actors) do
    if room == v.room then
      ...
      drawQueue[#drawQueue + 1] = {type = "actor", img = canvas, x = math.floor(x), y = math.floor(y), z= charHeight-tileSize, r = 100, g = 200, b = 100}
    end
  end
end
...
function queueScanChars(room)
  for i, v in ipairs(currentLevel.actors) do
    if room == v.room then
      ...
      local x, y = coordToIso(v.x, v.y)
      drawQueue[#drawQueue + 1] = {type = "actor", img = canvas, x = math.floor(x), y = math.floor(y), z= charHeight-tileSize, r = palette.green[1], g = palette.green[2], b = palette.green[3]}
    end
  end
end
